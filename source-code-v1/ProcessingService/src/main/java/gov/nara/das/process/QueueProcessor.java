...
public class QueueProcessor implements Runnable{
	...
	public final class MyCallable implements Callable<Long> {
		...
		public MyCallable(String aMessage, String messageId,List<Integer> aIndices, List<Integer> aLengths) {
			message = aMessage;
		...
		private void processMessage() {
            ...
            try {
            	...
            	for(int index:indices){
					...
					//log(messageBody);
					log("sending message to api. first 30 chars of message= "+(xml+"                                ").substring(0,30));
					ms.setHttpPostRequestTime(new Timestamp(System.currentTimeMillis()));
					
					ResponseEntity<CreateDescriptionResponse> r=httpService.postNew(xml);
					statusCode=r.getStatusCodeValue();
					CreateDescriptionResponse c=r.getBody();
					naid=c.getNaid();
					//
...
//					log("SAVED message status to db");
	...
	public QueueProcessor( HTTPService http, MessageStatusTemplate temp,MessageListener m,int anId,String url, int concurrent){
		queueUrl=url;
		id=anId;
		if(concurrent<=0){
			concurrent=5;
							...
							if (futures.size() < concurrentCount) {
								...
								Future future =null;
									callable = new MyCallable(message.getBody(), message.getMessageId(),
											getIntegerArrayListFromMessageAttributes(message, "indices")
											, getIntegerArrayListFromMessageAttributes(message, "lengths"));
...
private List<Integer> getIntegerArrayListFromMessageAttributes(Message m, String attributeName){
	...
	if(mv !=null){
		String s=mv.getStringValue();
		list=unflattenIntegerString(s);
		System.err.println("found integer list in attributes for key="+attributeName+", list.size="+list.size());
