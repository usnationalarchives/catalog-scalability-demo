		...
		private void processMessage() {
            ...
            try {
				...
				if (forwardBatches) {
					...
					log.debug("response newBatch=" + r1);
					return;
				}else{
					System.err.println("no batch processing.");
					log.debug("no batch processing.");
				}
            	int i=0;
            	...
            	for(int index:indices){
					...
					log.debug("response received from api. naid=" +naid);
					//
					log.debug("statusCode="+statusCode);
					log.debug("response received: "+System.currentTimeMillis() +" "+date2String(System.currentTimeMillis() ));
...
//					log("SAVED message status to db");
	...
	public void process() throws Exception  {
		...
		if (concurrentCount <= 0) {
			log.debug("ERROR concurrentCount="+concurrentCount);
			System.exit(1);
		} else {
		...
		try {
			...
			while (true) {
				...
				try {
					...
					log.debug("Error reading from SQS at " + queueUrl);
					e.printStackTrace();
				}
				if (rmr != null) {
						...
						while (true) {
							if (futures.size() < concurrentCount) {
								...
								Future future =null;
									...
									log.debug("Message was not deleted. So it won't be processed");
								}
								break;
							} else {
...
//			finished=true;
	...
	private synchronized void storeTime(long time){
		...
		log.debug("average thread time="+avg+"(ms)");
...
public synchronized boolean  deleteMessage(Message message){
	...
	try {
		...
		log.debug("processMessage(): failed to delete message " + message.getMessageId());;
		e.printStackTrace(System.err);
		// try one more time...
		try {
			sqsClient.deleteMessage(
			...
			log.debug("failed to delete message " + message.getMessageId());;
