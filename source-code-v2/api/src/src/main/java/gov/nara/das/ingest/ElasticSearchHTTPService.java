...
public class ElasticSearchHTTPService {
    ...
    public ESResponse postNew(Description description) {
    	...
    	//
    	log.debug("sending ES json="+json);
    	//
    	...
    	log.debug("receveived ES response="+response.getBody());
    	log.debug(" received ES response status code="+response.getStatusCodeValue());
    	//
    ...
    public ESResponse postSearch(String jsonQuery ) {
    	...
    	headers.setContentType(MediaType.APPLICATION_JSON);
    	//
    	log.debug("sending ES json="+jsonQuery);
    	//
