			...
			try {
				while (true) {
					...
					try{
						...
						log.debug("Exception caught while getting completion "
					...
					if(completed){
						log.debug("closing jobId="+job.getJobId());
						closeJob(job);
						break;
...
//
						log.debug("processing job=" + j);
						log.debug("retrieved_time=" + j.getRetrievedTime() + " for jobId=" + j.getJobId());
						log.debug("total=" + j.getTotalDescriptionCount() + " for jobId=" + j.getJobId());
						log.debug("success=" + j.getSuccessCount() + " for jobId=" + j.getJobId());
						log.debug("failed=" + j.getFailureCount() + " for jobId=" + j.getJobId());
